...
export OPENPI_DUQUANT_INCLUDE='(.*language_model.*(q_proj|k_proj|v_proj|o_proj|gate_proj|up_proj|down_proj).*|.*gemma_expert.*(gate_proj|up_proj|down_proj).*)'
# export OPENPI_DUQUANT_INCLUDE='.*paligemma_with_expert\.paligemma\.model\.language_model\.(?:.*\.)?(q_proj|k_proj|v_proj|o_proj|gate_proj|up_proj|down_proj).*'
# export OPENPI_DUQUANT_INCLUDE='.*paligemma_with_expert\.gemma_expert\.model\.(?:layers\.\d+\.)?(self_attn\.(q_proj|k_proj|v_proj|o_proj)|mlp\.(gate_proj|up_proj|down_proj)).*'
# Exclude: embeddings, normalization, vision tower, multi_modal_projector
...
export RESULTS_OUT_PATH="${RESULTS_OUT_PATH:-/home/jz97/VLM_REPO/openpi/results/libero_goal_w4a4}"
...
export OPENPI_DUQUANT_PACKDIR="/home/jz97/VLM_REPO/openpi/duquant_packed_full_llm_dit_mlp_w4a4_atm"
...
if [ -z "${ATM_ALPHA_PATH:-}" ]; then
    export ATM_ALPHA_PATH="/home/jz97/VLM_REPO/openpi/atm_alpha_llm_ditmlp_w4a4.json"
fi
...
time python examples/libero/main.py \
  ...
  --args.results-out-path "/home/jz97/VLM_REPO/openpi/results/libero_object_custom" \
  --args.seed "$SEED"
